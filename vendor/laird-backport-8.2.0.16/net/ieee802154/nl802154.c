...
static int nl802154_add_llsec_key(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (nla_parse_nested_deprecated(attrs, NL802154_KEY_ATTR_MAX, info->attrs[NL802154_ATTR_SEC_KEY], nl802154_key_policy, genl_info_extack(info)))
		return -EINVAL;
...
static int nl802154_del_llsec_dev(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (nla_parse_nested_deprecated(attrs, NL802154_DEV_ATTR_MAX, info->attrs[NL802154_ATTR_SEC_DEVICE], nl802154_dev_policy, genl_info_extack(info)))
		return -EINVAL;
...
static int nl802154_add_llsec_devkey(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!info->attrs[NL802154_ATTR_SEC_DEVKEY] ||
	    nla_parse_nested_deprecated(attrs, NL802154_DEVKEY_ATTR_MAX, info->attrs[NL802154_ATTR_SEC_DEVKEY], nl802154_devkey_policy, genl_info_extack(info)) < 0)
		return -EINVAL;
...
static int nl802154_del_llsec_devkey(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (nla_parse_nested_deprecated(attrs, NL802154_DEVKEY_ATTR_MAX, info->attrs[NL802154_ATTR_SEC_DEVKEY], nl802154_devkey_policy, genl_info_extack(info)))
		return -EINVAL;
